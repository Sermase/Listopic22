            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 relative -mt-32 z-10">
                <div className="flex flex-col md:flex-row items-end gap-6 mb-8">
                    ...
                    <div className="flex gap-3 mb-4 md:mb-2 w-full md:w-auto justify-center">
                        {isOwnProfile ? (
                            <button className="px-6 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white border border-white/10 transition-colors flex items-center gap-2">
                        ...
                        <>
                            {loadingLists ? (
                                ...
                                <div className="py-20 text-center border-2 border-dashed border-white/5 rounded-xl">
                                    <p className="text-gray-500 mb-4">Este usuario aún no ha creado ninguna lista.</p>
                                    {isOwnProfile && (
                                        <Link to="/create" className="text-indigo-400 hover:underline">Crear mi primera lista</Link>
                                    )}
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {lists.map(list => (
                                        <Link key={list.id} to={`/list/${list.id}`} className="block group">
                                            <div className="bg-[#151b2e] border border-white/10 rounded-xl overflow-hidden hover:border-indigo-500/50 transition-all">
                                                <div className="h-40 bg-gray-800 relative">
                                                    {list.photoUrl && <img src={list.photoUrl} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />}
                                                    {list.avgScore && <div className="absolute top-2 right-2 bg-black/60 px-2 py-1 rounded text-xs font-bold text-white">⭐ {list.avgScore.toFixed(1)}</div>}
                                                </div>
                                                <div className="p-4">
                                                    <h3 className="text-white font-bold text-lg mb-1 truncate">{list.name}</h3>
                                                    <p className="text-gray-500 text-xs mb-3">{list.itemCount || 0} lugares • {new Date((list.createdAt as any).seconds * 1000).toLocaleDateString()}</p>
                                                </div>
                                            </div>
                                        </Link>
                                    ))}
