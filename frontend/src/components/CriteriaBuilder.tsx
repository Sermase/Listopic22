    ...
    return (
        <div className="space-y-4">
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Sliders className="w-5 h-5 text-[var(--lt-accent)]" />
                ...
                {criteria.map((criterion) => {
                    ...
                    return (
                        <div key={criterion.id} className={`bg-[var(--lt-card-strong)] p-4 rounded-xl border ${isLocked ? 'border-[var(--lt-accent-border)]' : 'border-white/5'} animate-fade-in relative group`}>
                            ...
                            {isLocked && (
                                <div className="absolute top-2 right-2 px-2 py-0.5 bg-[var(--lt-accent-soft)] text-[var(--lt-accent)] text-[10px] font-bold rounded border border-[var(--lt-accent-border)] flex items-center gap-1">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            disabled={isLocked}
                                            className={`w-full bg-[var(--lt-bg)] border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[var(--lt-accent-border)] ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`}
                                        />
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4 mt-6 flex-wrap">
                                        ...
                                        <div className={`flex items-center gap-2 ${isLocked ? 'opacity-50' : ''}`}>
                                            ...
                                            <select
                                                ...
                                                disabled={isLocked}
                                                className={`bg-[var(--lt-bg)] border border-white/10 rounded px-2 py-1 text-white text-xs focus:outline-none focus:border-[var(--lt-accent-border)] ${isLocked ? 'cursor-not-allowed' : ''}`}
                                            >
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder="Ej: Malo"
                                            disabled={isLocked}
                                            className={`w-full bg-[var(--lt-bg)] border border-white/10 rounded-lg px-3 py-2 text-gray-300 text-xs focus:outline-none focus:border-[var(--lt-accent-border)] ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`}
                                        />
                                    </div>
                                    <div>
