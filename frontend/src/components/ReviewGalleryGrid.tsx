...
const ReviewFallback: React.FC<{ title: string; subtitle?: string }> = ({ title, subtitle }) => (
    <div className="w-full h-full relative">
        <PlacePhotoPlaceholder variant="review" compact />
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-1.5 pt-8 text-center">
            <span className="block text-[10px] sm:text-xs font-bold text-gray-200 line-clamp-1">
                {title}
            </span>
            {subtitle && (
                <span className="block text-[9px] text-gray-500 line-clamp-1 w-full mt-0.5">
                    {subtitle}
                </span>
            )}
        </div>
...
const ExpandedReview: React.FC<{
    ...
    reactionConfig?: { like?: string; dislike?: string };
    placeClosedStatus?: string;
}> = ({ review, onDelete, onEdit, onClose, reactionConfig, placeClosedStatus }) => {
    ...
    return (
        <div id={`profile-review-${review.id}`} ref={ref} className="col-span-3 md:col-span-4 lg:col-span-5 mb-4 scroll-mt-24 animate-[fade-in_0.2s_ease-out] flex flex-col">
            ...
            <ReviewCard
                ...
                placeClosedStatus={placeClosedStatus}
            />
        </div>
    );
};
...
export const ReviewGalleryGrid: React.FC<ReviewGalleryGridProps> = ({
    ...
    <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-1 sm:gap-2">
        {reviews.map((review: ReviewEntity) => {
            const reviewLike = review as ReviewEntity & { photos?: string[] };
            const firstPhoto = review.photoUrls?.[0] || review.photoUrl || reviewLike.photos?.[0] || null;
            ...
            const title = review.placeName || review.itemName || 'Lugar';
            const subtitle = review.itemName && review.itemName !== review.placeName ? review.itemName : undefined;
            ...
            if (isExpanded) {
                return (
                    <ExpandedReview
                        ...
                        onDelete={onDelete}
                        onEdit={onEdit}
                        onClose={() => setExpandedReviewIds(prev => prev.filter(id => id !== review.id))}
                        reactionConfig={reactionConfig}
                        placeClosedStatus={placeClosedStatus}
                    />
                );
                    ...
                    {photoUrl ? (
                        <>
                            <ProgressiveImage
                                ...
                                fallback={<ReviewFallback title={title} subtitle={subtitle} />}
                            />
                        ...
                        <ReviewFallback title={title} subtitle={subtitle} />
                    )}
