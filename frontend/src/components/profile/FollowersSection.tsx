...
export const FollowersSection: React.FC<FollowersSectionProps> = ({ targetUserId }) => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            ...
            setItems([]);
            try {
                const qIds = query(collection(db, 'users', targetUserId, 'followers'));
                ...
                for (const chunk of chunks) {
                    try {
                        ...
                        console.warn('Error fetching follower users chunk', e);
                    }
                }
                ...
                console.error('Error fetching follower users', err);
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="animate-fade-in">
            {loading ? (
                ...
                <div className="py-16 text-center border border-dashed border-white/5 rounded-xl bg-white/5">
                    ...
                    <p className="text-xs text-gray-600">Este usuario aún no tiene seguidores.</p>
                </div>
            ) : (
