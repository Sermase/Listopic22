...
export const AuthForm: React.FC<AuthFormProps> = ({ initialMode = 'login', onAuthenticated }) => {
    const { user } = useAuth();
    ...
    const handleEmailAuth = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (code === 'auth/wrong-password' || code === 'auth/invalid-credential') message = 'Correo o contraseña incorrectos.';
            if (code === 'auth/user-not-found') message = 'Usuario no encontrado.';
            if (code === 'auth/email-already-in-use') message = 'El correo ya está registrado.';
            if (code === 'auth/weak-password') message = 'La contraseña es muy débil (mínimo 6 caracteres).';
    ...
    return (
        <div className="w-full">
            <div className="text-center mb-6">
                ...
                <h1 className="text-2xl font-bold text-[var(--lt-text)] mb-2">
                    {isRegistering ? 'Crear cuenta' : 'Bienvenido de nuevo'}
                </h1>
                <p className="text-[var(--lt-text-muted)] text-sm">
                    {isRegistering ? 'Únete a la comunidad de Listopic' : 'Accede sin perder lo que estabas viendo'}
                </p>
            </div>

            {error && (
                <div className="bg-red-500/10 border border-red-500/20 text-red-200 p-3 rounded-lg text-sm mb-6 text-center" role="alert">
                    {error}
                </div>
            )}
                ...
                {isRegistering && (
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-[var(--lt-bg)] border border-white/10 rounded-xl px-10 py-3 text-[var(--lt-text)] placeholder-gray-500 focus:outline-none focus:border-[var(--lt-accent-border)] transition-colors"
                            required
                        />
                    </div>
                )}
            ...
            <div className="mt-6 text-center">
                <button
                    ...
                    className="text-[var(--lt-accent)] text-sm font-medium hover:underline transition-colors"
                >
